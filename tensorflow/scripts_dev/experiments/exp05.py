...
class Flags:
    def __init__(self):
        ...
        self.summaries_dir = "./exp05/tensorboard"
...
with tf.device("/gpu:0"):
    ...
    with tf.Graph().as_default():
        ...
        images_placeholder = tf.placeholder(tf.float32, shape=(None, IMAGE_PIXELS), name="net_input")
        # Hidden 1
        ...
        # Linear
        labels_placeholder = tf.placeholder(tf.int32, shape=(None), name="net_labels")
        ...
        synthetic_gradients_labels_placeholder = tf.placeholder(tf.int32, shape=(None), name="synt_grad_labels")
        ...
        with tf.name_scope('synthetic_gradients'):
            #forward pass
            labels_one_hot = tf.one_hot(synthetic_gradients_labels_placeholder, FLAGS.num_classes)
            labels_one_hot.set_shape([None, FLAGS.num_classes])
            
            input_and_labels = tf.concat([synthetic_gradients_input_message, labels_one_hot], 1)
            
            synthetic_gradients_model_input_and_labels_shape = tf.shape(input_and_labels)
            
            batch_dimension = synthetic_gradients_model_input_and_labels_shape[0] # batch dimension
            
            synthetic_gradients_model_input_layer_state = tf.concat([
                synthetic_gradients_input_weights,
                tf.expand_dims(synthetic_gradients_input_biases,0)
            ],0)
            
            synthetic_gradients_model_input_layer_state_tiled = tf.tile(
                tf.reshape(synthetic_gradients_model_input_layer_state,[1,-1]), 
                tf.stack([batch_dimension, 1])
            )
            
            synthetic_gradients_model_input = tf.concat([
                input_and_labels,
                synthetic_gradients_model_input_layer_state_tiled
            ],1)
            

            synthetic_gradients_model_input_width = synthetic_gradients_model_input.get_shape().as_list()[1]
            synthetic_gradients_model_output_width = hidden2_true_gradients.get_shape().as_list()[1]
            synthetic_gradients_model_weights = tf.Variable(
                tf.zeros([synthetic_gradients_model_input_width, synthetic_gradients_model_output_width]),name='weights')
            synthetic_gradients_model_biases = tf.Variable(tf.zeros([synthetic_gradients_model_output_width]), name='biases')
            ...
            synthetic_gradients_model_output = synthetic_gradients_model_logits

            #backward pass
            true_gradients_placeholder = tf.placeholder(tf.float32, 
                                                        shape=hidden2_true_gradients.get_shape().as_list(), name="hidden2_true_gradients")

            # Mean squared error 
            synthetic_gradients_model_loss = tf.reduce_sum(
                tf.pow(true_gradients_placeholder - synthetic_gradients_model_output, 2))/(2*tf.to_float(batch_dimension))
